...
const screens = TOUCHABLES.reduce((map, obj) => ((map[obj.text] = obj), map), {})
...
export class TouchableExample extends Component {
  ...
  toggleScrollView = () => this.setState(prev => ({ useScrollView: !prev.useScrollView }))

  render() {
    const { type: GHTouchable, background, props, renderChild, text, color } = screens[
    ...
    return (
      <Component style={{ width: '100%', height: '100%', padding: 10 }}>
        <TouchableOpacity onPress={this.toggleScrollView}>
          <Text>Use {this.state.useScrollView ? 'View' : 'ScrollView'} as a wrapper</Text>
        </TouchableOpacity>
        <Text>{text}</Text>
        <View
          style={{
          ...
          <RNTouchable {...props} background={background && background(RNTouchableNativeFeedback)}>
            {renderChild(color)}
          </RNTouchable>
          <GHTouchable {...props} background={background && background(TouchableNativeFeedback)}>
            {renderChild(color)}
          </GHTouchable>
        </View>
      </Component>
